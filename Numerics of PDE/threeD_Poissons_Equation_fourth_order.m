...
% Lab File 08  
% Numerical Solution of 3D Poissons Equation using o(h^4) method 
  ...
  % Formation of Nodal Points 
    ...
    for N=2:NN+1     
         for M=2:MM+1 
            ...
            F5=F(L,M,N+1); 
            F6=F(L,M,N-1); 
            FF=0.5*H*H*(F1+F2+F3+F4+F5+F6+6*F0); 
            U0=U(L,M,N); 
            U1=U(L+1,M,N); 
            U2=U(L-1,M,N); 
            U3=U(L,M+1,N); 
            ...
            U17=U(L+1,M,N-1); 
            U18=U(L-1,M,N-1); 
            AA=6*(U1+U2+U3+U4+U5+U6); 
            BB1=U7+U8+U9+U10-2*(U1+U2+U3+U4); 
            BB2=U11+U12+U13+U14-2*(U3+U4+U5+U6); 
            BB3=U15+U16+U17+U18-2*(U5+U6+U1+U2); 
            U(L,M,N)=(AA+BB1+BB2+BB3-FF)/24; 
            end 
         end 
