...
%NUMERICAL SOLUTION OF 3D HEAT CONDUCTION EQUATION 
%ADI METHOD
%Uxx+Uyy+Uzz=Ut, U(x,y,t)=exp(-3*pi*pi*t)*sin(pi*x)*sin(pi*y)*sin(pi*z)
%------------------------------------------------------------------------
clear all;
clc;
lam=1.6;
...
for l=1:ll+1
    for m=1:mm+1
        for n=1:nn+1
          for j=1:jj+1
              EU(l,m,n,j)=exp(-3*pi*pi*T(j))*sin(pi*X(l))*sin(pi*Y(m))*sin(pi*Z(n));
          end
        end
    end
end
    ...
    for l=2:ll
        for m=2:mm
            for n=2:nn
                ...
                DIAG(n)=1+lam;
                SUP(n)=-0.5*lam;
                RH(n)=R0+0.5*lam*(R34+R35+R36)+0.25*lam*lam*(R41+R42+R43)+0.125*lam*lam*lam*R44;
            end
