...
%LAB = 01.    %MATLAB FILE:- 03.      %DATE=06.08.2024
%SOR METHOD.
...
for IA=1:30 
    OMEGA=1.40+0.01*IA; 
    disp('Numerical Solution of 2D Poissons Equation-SoR method'); 
    disp('   '); 
    fprintf('LL=%4.2f,OMEGA=%8.6f\n',LL,OMEGA); 
    disp('   '); 
    fprintf('LL=%4.2f,OMEGA=%8.6f\n',LL,OMEGA); 
    disp('  '); 
    ...
    for KK=1:6000 
        ...
        for M=2:MM+1 
            for L=2:LL+1 
                ...
                U3=U(L,M+1); 
                U4=U(L,M-1); 
                RH=OMEGA*(U1+U2+U3+U4-H*H*F0)+(1-OMEGA)*4*U0; 
                U(L,M)=RH/4; 
            end 
        end 
