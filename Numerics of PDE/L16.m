%--------------------------------
% Numerical Solution of 2D Nonlinear Parabolic Eqn with forcing function
% Uxx+Uyy=Ut+alpha*U*U+f(x,y,t), % U(x,y,t)=exp(-t)*cos(pi*x)*cos(pi*y)
% Name:-KSHITIJ BAHUKHANDI;
% Roll No:- SAU/AM(M)/2023/06
% APM (III-Semester)-2024-25
...
%-----------------------------
 ...
 JJ=80; % JJ=160, JJ=640, JJ=2560, %Solution at t=1
 H=1.0/LL;
 K=LAM*H*H;
 ...
 U=zeros(LL+1,MM+1,JJ+1);EU=zeros(LL+1,MM+1,JJ+1);
 ...
 F(L,M,J)=(1-2*pi*pi)*ETT(L,M,J)-alpha*ETT(L,M,J)^2;
 end
 end
 end
 % Initial conditions
 ...
 P0=0.25*alpha*K;
 P1=1+2*LAM+2*P0*R0;
 P2=K*F0+P0*R0*R0-R0-0.5*LAM*(R5+R6+U5+U6);
