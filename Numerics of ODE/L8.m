...
% using Padé Approximation on a Variable Mesh
%
...
%
% Numerical Method:
%     Padé Approximation Method with Variable Mesh
%
% Language:
%     MATLAB
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  N=10; 
  ETA=1.2;
  disp('Variable Mesh Numerical Solution for IVP Using Exponential Approach');
  disp('    ');
  fprintf('N=%4.2f\n',N);
  disp('  ');
  for I=1:8
 ...
 %   U(1)=EU(1);
    ...
    % Solution at advanced time
       for L=1:N
           ...
           C=LAMDA*H0;
           % Second Order Approx
             D1=1+0.5*C;  D2=1-0.5*C;
           %  D=D1/D2;
             % Fourth Order Approx
             D1=1+0.5*C+(C*C/12);   D2=1-0.5*C+(C*C/12);
             D=D1/D2;
    %----
       U(L+1)=D*U(L);
       end 
