...
% using the Mid-Point (Nyström) Method
%
...
clear all;  % clear all variables in memory
  ...
  for I=1:8
  H=1.0/N;
  disp('Numerical Solution of system of IVPs Using Mid-point Method');
  disp('    ');
  ...
  %  U(1)=EU(1); U(2)=EU(2);
  %  V(1)=EV(1); V(2)=EV(2);
    U(1)=1.0; U(2)=1+H+0.5*H*H;
    V(1)=2.0; V(2)=2*(1+H+0.5*H*H);
     tic  % CPU time starts
    % Solution at advanced time
       for L=2:N   
       U(L+1)=U(L-1)+2*H*(3*U(L)-V(L)); 
       V(L+1)=V(L-1)+4*H*U(L);
       end 
  % Calculate Errors
