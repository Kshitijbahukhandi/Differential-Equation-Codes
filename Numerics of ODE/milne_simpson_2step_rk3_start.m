...
%
% Initialization:
%     Third-Order Classical Explicit Runge-Kutta (RK3) Method used
%     to compute the starting value at t = h
%
% Numerical Method:
%     Two-Step Milne-Simpson Predictor-Corrector Method
%
% Language:
%     MATLAB
%
...
NU(1)=EU(1);
tic % CPU time starts
% Solution at t=h using 3rd order classical explict RK-Method
K1=-2*H*T(1)*U(1)*U(1);
K2=-2*H*(T(1)+H/2)*(U(1)+K1/2)*(U(1)+K1/2);
K3=-2*H*(T(1)+H)*(U(1)+K2/2)*(U(1)+K2/2);
...
% fprintf('N=%4.2f, ERU=%10.4e/n',N,ERU);
% disp(' ');
