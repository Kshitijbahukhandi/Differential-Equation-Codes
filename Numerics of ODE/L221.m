...
% using the Midpoint (Nyström) Method on a Variable Mesh
%
...
%
% Numerical Method:
%     Midpoint (Nyström) Method with Variable Mesh
%
% Language:
%     MATLAB
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  N=10; 
  ETA=1.2;
  disp('Variable Mesh Numerical Solution for the system of IVPs Using Mid-point Method');
  disp('    ');
  fprintf('N=%4.2f\n',N);
  disp('  ');
  for I=1:8
 ...
 %   U(1)=EU(1); U(2)=EU(2);
 %   V(1)=EV(1); V(2)=EV(2);
    U(1)=1.0; U(2)=1+H(2)+0.5*H(2)*H(2);
    V(1)=2.0; V(2)=2*(1+H(2)+0.5*H(2)*H(2));
     tic  % CPU time starts
    % Solution at advanced time
       for L=2:N
           H0=H(L+1); HH0=H(L);
       U(L+1)=U(L-1)+(H0+HH0)*(3*U(L)-V(L)); 
       V(L+1)=V(L-1)+2*(H0+HH0)*U(L);
       end 
  % Calculate Errors
