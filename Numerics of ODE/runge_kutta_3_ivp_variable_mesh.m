...
clear all;  % clear all variables in memory
  N=10; 
  ETA=1.2;
  disp('Variable Mesh Numerical Solution for IVP Using 3rd order explicit RK Method');
  disp('    ');
  fprintf('N=%4.2f\n',N);
  disp('  ');
  for I=1:8
 ...
 %   U(1)=EU(1);
    ...
    % Solution at advanced time
       for L=1:N
           ...
           K3= -2*H0*(T0+H0)*(U0-K1+2*K2)*(U0-K1+2*K2);
           U(L+1)=U(L)+(K1+4*K2+K3)/6;
       %---Nystrom Method
           K1= -2*H0*T0*U0*U0;
           K2= -2*H0*(T0+2*H0/3)*(U0+2*K1/3)*(U0+2*K1/3);
           K3= -2*H0*(T0+2*H0/3)*(U0+2*K2/3)*(U0+2*K2/3);
           U(L+1)=U(L)+(2*K1+3*K2+3*K3)/8;     
       end 
  % Calculate Errors
